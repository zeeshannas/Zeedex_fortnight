...
ORDER BY KU.TABLE_NAME, KU.ORDINAL_POSITION;");
    ...
    public function splitFilterValue($filterValue) {
        $driverName = $this->getDriverName();
        $LIKE_OPERATOR = $driverName === 'Postgresql' ? 'ILIKE' : 'LIKE';
    ...
    public function filtering(Select $select, $filterType = PredicateSet::OP_AND)
    {
        ...
        foreach ($filters as $filterName => $filterValue) {
            ...
            if ($this->isFieldWithRelationNtoN($filterName)) {
                // If the filter is relationNtoN we will filter that within a second query later
                continue;
            } else if (!$this->isFieldFrom1to1Relation($filterName)) {
    ...
    public function update($primaryKeyValue, $data, $tableName = null) {
        ...
        $dbTableName = $tableName === null ? $this->tableName : $tableName;
        $isPrimaryTable = $tableName === null;

        $primaryKeyField = $this->getPrimaryKeyField($tableName);

        // First a validation check that we have something to update!
        $sql = new Sql($this->adapter);
        $select = $sql->select()
            ->columns([$primaryKeyField])
            ->from($this->getTableNameWithSchema($dbTableName));
        ...
        $select->where([
            $dbTableName . '.' . $primaryKeyField . ' = ?' => $primaryKeyValue
        ]);
        if ($this->_where !== null && $isPrimaryTable) {
        ...
        $sql = new Sql($this->adapter);
        $update = $sql->update($this->getTableNameWithSchemaForOperations($dbTableName));
        $update->where([
    ...
    public function getOne($id, $tableName = null)
    {
        $sql = new Sql($this->adapter);
        $select = $sql->select();
        ...
        if (empty($dbTableName)) {
            throw new \Exception("The tableName can't be empty");
        }

        $select->from($this->getTableNameWithSchema($dbTableName));

        $primaryKeyField = $this->getPrimaryKeyField($dbTableName);
    ...
    public function setRelations1To1($dbRelations) {
        $this->_relation_1_1 = $dbRelations;
    }
    ...
    public function isFieldFrom1to1Relation($fieldName)
    {
        foreach ($this->_relation_1_1 as $relation) {
            foreach ($relation->relatedTableFields as $relatedTableFieldName) {
                if ($relatedTableFieldName === $fieldName) {
                    return true;
                }
            }
        }
    ...
    public function joinStatements($select)
    {
        ...
        foreach ($this->_relation_1_1 as $relation) {
            ...
            $shortNameForRelationTable = $this->getShortName($relation->relatedTable, $relation->relationFieldName);

            $select->join(
                [$shortNameForRelationTable => $this->getTableNameWithSchemaForOperations($relation->relatedTable)],
                $this->tableName . '.' . $relation->relationFieldName . ' = ' . $shortNameForRelationTable . '.' . $relation->relatedPrimaryField,
                $relation->relatedTableFields,
                Select::JOIN_LEFT
            );
        }
    ...
    public function setColumns($columns) {
        ...
        foreach ($this->_relation_1_1 as $relation) {
            foreach ($relation->relatedTableFields as $relatedFieldName) {
                $columnsKey = array_search($relatedFieldName, $columns);

                // Unsetting the related field name from the columns array
                // since this will be joined later
                if ($columnsKey !== false) {
                    unset($columns[$columnsKey]);
                }
            }
        }
