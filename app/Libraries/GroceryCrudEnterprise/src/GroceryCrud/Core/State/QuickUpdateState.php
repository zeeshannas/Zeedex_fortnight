...
class QuickUpdateState extends StateAbstract {
    ...
    public function render()
    {
        ...
        if (empty($stateParameters->primaryKeyValue) || empty($stateParameters->fieldName)) {
            throw new \Exception('Primary key value or field name is missing.');
        }
        ...
        $validator = $this->setValidationRules('quick-update', $stateParameters->fieldName);
        ...
        if (!$validator->validate()) {
            ...
            try {
                ...
                // Filter to only have an array of fields with the quick edit fields
                $editFields = array_filter($editFields, function ($field) use ($stateParameters) {
                    return $field->name === $stateParameters->fieldName;
                });
                ...
                $uploadedData = $this->uploadOneData($fieldTypes, $stateParameters->fieldName);
                if (!empty($uploadedData)) {
                    $stateParameters->data = $this->addUploadOneData($stateParameters->data, $uploadedData);
                }

                $uploadedMultipleData = $this->uploadMultipleData($fieldTypes, $stateParameters->fieldName);
                if (!empty($uploadedMultipleData)) {
                    $stateParameters->data = $this->addUploadMultipleData($stateParameters->data, $uploadedMultipleData);
                }

                $uploadOneFields = $this->getUploadOneFields($fieldTypes, $stateParameters->fieldName);
                $uploadMultipleFields = $this->getUploadMultipleFields($fieldTypes, $stateParameters->fieldName);
