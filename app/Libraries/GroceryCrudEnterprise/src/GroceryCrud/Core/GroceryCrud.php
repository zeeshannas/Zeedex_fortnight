 ...
 * @copyright  Copyright (c) 2010 through 2024, John Skoubourdis
 * @license    LICENSE.md
 * @version    3
...
class GroceryCrud implements GroceryCrudInterface
{
    ...
    const FIELD_TYPE_PASSWORD_TOGGLE = 'password_toggle';
    ...
    /**
     * Grouping fields in the form for all form operations
     *
     * @var array
     */
    ...
    /**
     * Grouping fields in the form for the read form
     *
     * @var array
     */
    protected $_groupFieldsReadForm = [];

    /**
     * Grouping fields in the form for the add form
     *
     * @var array
     */
    protected $_groupFieldsAddForm = [];

    /**
     * Grouping fields in the form for the edit form
     *
     * @var array
     */
    protected $_groupFieldsEditForm = [];


    /**
     * Grouping fields in the form for the clone form
     *
     * @var array
     */
    protected $_groupFieldsCloneForm = [];
     ...
     * @var bool
     */
    protected $_load_list = true;

    /**
     * @var bool
     */
    protected $_load_back_to_list = true;
     ...
     * @var array
     */
    protected $_fieldTypesColumns = [];

    /**
     * @var array
     */
    protected $_fieldTypesSeachColumns = [];
     ...
     * @var array
     */
    protected $_unset_sorting_columns = [];

    /**
     * @var array
     */
    protected $_inlineEditFields = [];
    ...
    /**
     * Group fields in the add form.
     *
     * This method allows grouping fields in the add form by specifying a display name.
     * If an array of display names is provided, it will iterate through each and set the group fields.
     *
     * @param string|array $displayAs The display name or an array of display names for the group.
     * @param array $fields The fields to be grouped under the specified display name.
     * @return $this
     */
    public function groupFieldsAddForm($displayAs, array $fields = [])
    {
        if (is_array($displayAs)) {
            foreach ($displayAs as $key => $value) {
                $this->_groupFieldsAddForm[] = (object)[
                    "displayAs" => $key,
                    "fields" => $value,
                ];
            }
     ...
     *
     * This method allows grouping fields in the edit form by specifying a display name.
     * If an array of display names is provided, it will iterate through each and set the group fields.
     *
     * @param string|array $displayAs The display name or an array of display names for the group.
     * @param array $fields The fields to be grouped under the specified display name.
     * @return $this
     */
    public function groupFieldsEditForm($displayAs, array $fields = [])
    {
        if (is_array($displayAs)) {
            foreach ($displayAs as $key => $value) {
                $this->_groupFieldsEditForm[] = (object)[
                    "displayAs" => $key,
                    "fields" => $value,
                ];
            }
    ...
    /**
     * Group fields in the clone form.
     *
     * This method allows grouping fields in the clone form by specifying a display name.
     * If an array of display names is provided, it will iterate through each and set the group fields.
     *
     * @param string|array $displayAs The display name or an array of display names for the group.
     * @param array $fields The fields to be grouped under the specified display name.
     * @return $this
     */
    public function groupFieldsCloneForm($displayAs, array $fields = [])
    {
        if (is_array($displayAs)) {
            foreach ($displayAs as $key => $value) {
                $this->_groupFieldsCloneForm[] = (object)[
                    "displayAs" => $key,
                    "fields" => $value,
                ];
            }
    ...
    /**
     * Group fields in the read form.
     *
     * This method allows grouping fields in the read form by specifying a display name.
     * If an array of display names is provided, it will iterate through each and set the group fields.
     *
     * @param string|array $displayAs The display name or an array of display names for the group.
     * @param array $fields The fields to be grouped under the specified display name.
     * @return $this
     */
    public function groupFieldsReadForm($displayAs, array $fields = [])
    {
        if (is_array($displayAs)) {
            foreach ($displayAs as $key => $value) {
                $this->_groupFieldsReadForm[] = (object)[
                    "displayAs" => $key,
                    "fields" => $value,
                ];
            }
    ...
    /**
     * Group fields for all form operations.
     *
     * This method allows grouping fields for all form operations (add, edit, clone, read)
     * by specifying a display name per group.
     *
     * @param string|array $displayAs The display name or an array of display names for the group.
     * @param array $fields The fields to be grouped under the specified display name.
     * @return $this
 */
    public function groupFields($displayAs, array $fields = [])
    {
        if (is_array($displayAs)) {
            foreach ($displayAs as $key => $value) {
                $this->_groupFields[] = (object)[
                    "displayAs" => $key,
                    "fields" => $value,
                ];
            }
    ...
    /**
     * Get the group fields for all form operations
     *
     * @return array
     */
    ...
    /**
     * Get the group fields for the add form
     *
     * @return array
     */
    public function getGroupFieldsAddForm(): array
    {
        return $this->_groupFieldsAddForm;
    }

    /**
     * Get the group fields for the edit form
     *
     * @return array
     */
    public function getGroupFieldsEditForm(): array {
        return $this->_groupFieldsEditForm;
    }

    /**
     * Get the group fields for the clone form
     *
     * @return array
     */
    public function getGroupFieldsCloneForm(): array {
        return $this->_groupFieldsCloneForm;
    }

    /**
     * Get the group fields for the read form
     *
     * @return array
     */
    public function getGroupFieldsReadForm(): array {
        return $this->_groupFieldsReadForm;
    }
    ...
    public function fieldTypeColumn($columnName, $columnType, $permittedValues = null, $options = null) {
        if ($columnType instanceof ModelFieldType) {
            $this->_fieldTypesColumns[$columnName] = $columnType;
            ...
            $this->_fieldTypesColumns[$columnName] = $columnTypeModel;
        } else {
     ...
     * @throws \Exception
     */
    public function fieldTypeSearchColumn($columnName, $columnType, $permittedValues = null, $options = null) {
        if ($columnType instanceof ModelFieldType) {
            $this->_fieldTypesSeachColumns[$columnName] = $columnType;
        } else if (is_string($columnType)) {
            $columnTypeModel = new ModelFieldType();
            $columnTypeModel->setDataType($columnType);
            $columnTypeModel->setPermittedValues($permittedValues);
            ...
            $this->_fieldTypesSeachColumns[$columnName] = $columnTypeModel;
        } else {
            throw new \Exception('Unrecognized type of the variable $columnType with value:' . $columnType);
        }
     ...
     * @return array
     */
    public function getFieldTypeColumns() : array {
        return $this->_fieldTypesColumns;
    }
    ...
    public function getFieldTypesSearchColumns() : array {
        return $this->_fieldTypesSeachColumns;
    }
    ...
    /**
     * Return the fields that doesn't exist in the database. Also called as virtual fields.
     *
     * @return array
     */
     ...
     * @return $this
     */
	public function fieldType($fieldName , $fieldType, $permittedValues = null, $options = null)
	{
        switch ($fieldType) {
            case 'virtual':
                $this->_virtualFields[] = $fieldName;

                break;

            default:
                $this->_field_types[$fieldName] = (object)[
                    'fieldName' => $fieldName,
                    'dataType' => $fieldType,
     ...
     * @return $this
     */
    public function fieldOptions(string $fieldName, array $options): GroceryCrud
    {
    ...
    /**
     * Set a 1-to-1 database relation.
     * This method defines a one-to-one relationship between the current table and a related table.
     *
     * @param string $relationFieldName
     * @param string $relatedTable
     * @param array $relatedTableFields
     * @param string|null $relatedPrimaryField (optional) If not set, the primary field of the related table will be used
     * @return $this
     */
    public function setRelation1to1($relationFieldName, $relatedTable, $relatedTableFields)
    {
        $this->_relation1to1[$relationFieldName] = (object)[
            'relationFieldName' => $relationFieldName,
            'relatedTable' => $relatedTable,
            'relatedTableFields' => $relatedTableFields,
            'relatedPrimaryField' => null
        ];
     ...
     * @return array
     */
    public function getRelation1to1()
    {
        return $this->_relation1to1;
    }
	...
	{
        $this->_unsetAddFields = array_merge($this->_unsetAddFields, $fields);
		return $this;
	}
	...
	{
        $this->_unset_columns = array_merge($this->_unset_columns, $columns);
		return $this;
	}
	...
	{
        $this->_unsetEditFields = array_merge($this->_unsetEditFields, $fields);
		return $this;
	}
    ...
    {
        $this->_unset_clone_fields = array_merge($this->_unset_clone_fields, $fields);
        return $this;
    }
    ...
    {
        $this->_unset_readFields = array_merge($this->_unset_readFields, $fields);
        return $this;
    }
    ...
    public function setList(): GroceryCrud {
        $this->_load_list = true;
        return $this;
    }

    /**
     * Unset the data list from the CRUD. This operation will remove any frontend sections for the list and will
     * also disable the access to the data list via the API.
     *
     * @return $this
     */
    public function unsetList(): GroceryCrud {
        $this->_load_list = false;
        return $this;
    }

    public function getLoadList(): bool {
        return $this->_load_list;
    }

    public function setBackToList()
    {
        $this->_load_back_to_list = true;
        return $this;
    }

    /**
     * Unset all the "back to datagrid" buttons and messages, such as "Close" or "Close modal on save"
     * options. The usual case of this function is when you have standalone forms.
     *
     * @return $this
     */
    public function unsetBackToList(): GroceryCrud
    {
        $this->_load_back_to_list = false;
        return $this;
    }

    public function getLoadBackToList(): bool {
        return $this->_load_back_to_list;
    }
     ...
     * @return $this
     */
    public function setDefaultState($stateName, $stateParameters = null): GroceryCrud {
        $this->_defaultState = (object)[
            'stateName' => $stateName,
    ...
    /**
     * Ability to have quick inline edit fields on table rows.
     *
     * @param array $fields
     * @return $this
     */
    public function inlineEditFields(array $fields): GroceryCrud
    {
        $this->_inlineEditFields = $fields;
        return $this;
    }

    /**
     * Get the inline edit fields.
     *
     * @return array
     */
    public function getInlineEditFields(): array {
        return $this->_inlineEditFields;
    }
}
